...
export default function AdicionarPrevisao() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setPrevisao({ id: 0, nome: "", tamanho: 0, estado: "", consumo: 0, retorno: 0 })
    ...
    return (
        <main className="bg-[url('.././public/blur.png')] bg-no-repeat min-w-screen m-auto flex flex-col">
            <Link href={"/seus_resultados"} className="text-white font-bold p-4 text-2xl"><FontAwesomeIcon icon={faArrowLeft} /></Link>
                ...
                <div className="bg-white rounded-2xl flex flex-col mb-56 mt-20 ml-auto mr-auto w-1/4">
                    ...
                    <form onSubmit={handleSubmit} className="p-4">
                        <div className="flex flex-col p-2">
                            ...
                            <input className="border border-gray-700 p-1 rounded-md" type="text" required name="nome" value={previsao.nome} id="idnome"
                                onChange={handleChange} />
                        </div>
                        <div className="flex flex-col p-2">
                            <label className="text-gray-700" htmlFor="idgeracao">Tamanho Disponível(ha):</label>
                            <input className="border border-gray-700 p-1 rounded-md" type="number" required name="tamanho" value={previsao.tamanho} id='idgeracao'
                                onChange={handleChange} />
                        </div>
                        <div className="flex flex-col p-2">
                            <label className="text-gray-700" htmlFor="idpreco">Estado(UF):</label>
                            <input className="border border-gray-700 p-1 rounded-md" type="text" required name="estado" value={previsao.estado} id="idpreco"
                                onChange={handleChange} />
                        </div>
                        <div className="flex flex-col p-2">
                            <label className="text-gray-700" htmlFor="idestoque">Consumo(kwh/m):</label>
                            <input className="border border-gray-700 p-1 rounded-md" type="number" required name="consumo" id="idestoque" value={previsao.consumo}
                                onChange={handleChange} />
                        </div>
